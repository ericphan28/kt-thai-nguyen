...
export function StudentList() {
  ...
  const allSoThich = Array.from(
    new Set(students.flatMap(student => student.soThich))
  ).sort()

  // Filter students based on search and filter criteria
  const filteredStudents = students.filter(student => {
    const matchSearch = student.hoTen.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       student.mssv.includes(searchTerm) ||
                       student.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
                       student.noiO.toLowerCase().includes(searchTerm.toLowerCase())
    
    const matchFilter = !filterSoThich || student.soThich.includes(filterSoThich)
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a 
                      ...
                      title="Gọi Zalo"
                    >
                      {student.tenHienThiZalo}
                    </a>
                    <span className="text-gray-500">({student.sdtZalo})</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>Sinh năm {student.namSinh}</span>
                </div>
                
                <div className="flex items-center gap-2 text-sm">
                  <Briefcase className="w-4 h-4 text-purple-500" />
                  <span className="font-medium">Công việc: {student.congViec}</span>
                </div>
                
                <div className="flex items-center gap-2 text-sm">
                  <MapPin className="w-4 h-4 text-red-500" />
                  <span>{student.noiO}</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 mt-4">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-48">
                    ...
                    <DropdownMenuItem onClick={() => window.open(`tel:${student.sdtZalo}`)}>
                      <MessageCircle className="w-4 h-4 mr-2 text-green-500" />
                      Gọi Zalo
                    </DropdownMenuItem>
                    {student.linkFacebook && (
                      <DropdownMenuItem onClick={() => window.open(student.linkFacebook, "_blank")}>
